...
def Todos(user_id, collection1, collection2):
    ...
    for i in result_todo:
        ...
        count+= 1           # hint -> 업무 번호가 1씩 증가하므로 +=3 -> +=1로 변경
    print("")

    # todo중 하나 입력
    user_input = int(input("Title 번호: "))-1    # hint -> input 값은 str이기 때문에 -1을 하려면 int로 바꿔줘야 함.
    # Status 입력
    user_status = input("Status: ")    # hint -> status는 int가 아니므로 int 삭제
